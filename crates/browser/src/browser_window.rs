use gpui::prelude::*;
use gpui::{
    div, px, App, Context, Entity, EventEmitter, FocusHandle, Focusable, Render,
    StyleRefinement, Styled, ViewContext, Window
};
use gpui_component::{
    TabBar, Tab, TextInput, InputState, Icon,
    IconName, Size, Sizable,
    h_flex, v_flex, ActiveTheme, StyledExt
};
use gpui_webview::{WebView, events::*};

use crate::tab_manager::{TabManager, BrowserTab};
        ...
        let tabs: Vec<Tab> = self.tab_manager
            ...
            .map(|(index, tab)| {
                ...
                Tab::new(&tab.title)
                    ...
                    .when(tab.is_loading, |this| this.prefix(Icon::new(IconName::Loader2)))
                    .suffix(
                        div()
                            .child(Icon::new(IconName::X).small())
                            .cursor_pointer()
                            .p_1()
                            .rounded_sm()
                            .hover(|style| style.bg(cx.theme().ghost_element_hover))
                            .on_click(cx.listener(move |this: &mut BrowserWindow, _, _, cx| {
